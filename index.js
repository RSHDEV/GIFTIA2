...
async function starts() {
	...
	fs.existsSync('./corpwa.json') && corp.loadAuthInfo('./corpwa.json')
	corp.on('connecting', () => {
		start('2', 'Connecting...')
	})
	corp.on('open', () => {
	...
	await corp.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./corpwa.json', JSON.stringify(corp.base64EncodedAuthInfo(), null, '\t'))
	...
	corp.on('chat-update', async (mek) => {
		try {
			...
			switch(command) {
				case 'help':
				case 'menu':
					corp.sendMessage(from, help(prefix), text)
					break
				case '#menu2':
					const buttons = [
				...
				case 'info':
					...
					corp.sendMessage(from, buffer, image, {caption: teks, contextInfo:{mentionedJid: [me.jid]}})
					break
				case 'blocklist':
				...
				case 'sticker':
					...
					break
				case 'gtts':
					if (args.length < 1) return corp.sendMessage(from, 'Kode bahasanya mana om?', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return corp.sendMessage(from, 'Textnya mana om', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					dtt.length > 600
					? reply('Textnya kebanyakan om')
					: gtts.save(ranm, dtt, function() {
						corp.sendMessage(from, fs.readFileSync(ranm), audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
						fs.unlinkSync(ranm)
					})
					break
				case 'meme':
				...
				/*case 'loli':
					loli.getSFWLoli(async (err, res) => {
						...
						corp.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ingat! Citai Lolimu'})
					})
					break
				case 'nsfwloli':
					...
					loli.getNSFWLoli(async (err, res) => {
						if (err) return reply('❌ *ERROR* ❌')
						buffer = await getBuffer(res.url)
						corp.sendMessage(from, buffer, image, {quoted: mek, caption: 'Jangan jadiin bahan buat comli om'})
					})
					break
				case 'hilih':
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							corp.sendMessage(_.jid, buff, image, {caption: `[ Ini Broadcast ]\n\n${body.slice(4)}`})
						}
